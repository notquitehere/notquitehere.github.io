...
Due to the magic of Pandoc we can straight up copy across the macros and header formatting without changes other than to add `\vspace{-4ex}` to the header `before-code` in order to reduce the amount of whitespace to a more sensible level. The macros can then be used in our markdown document as they are.
